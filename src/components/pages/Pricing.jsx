    ...
    <div className="max-w-7xl mx-auto relative z-10">
      ...
      <div className="text-center mb-20">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold text-white tracking-tight leading-[1.1] mb-8">
          Plans for every <span className="bg-gradient-to-r from-[#197DFF] to-[#22A756] bg-clip-text text-transparent italic">investor</span>
        </h1>
        ...
        {plans.map((p, i) => (
          <div 
            ...
            className={`group relative rounded-[2.5rem] p-10 md:p-12 backdrop-blur-xl border transition-all duration-500 hover:-translate-y-2 flex flex-col h-full
              ${p.highlight 
                ? 'bg-[#111318]/90 border-[#22A756]/30 shadow-[0_20px_50px_rgba(34,167,86,0.1)]' 
                : 'bg-[#111318]/60 border-white/5 hover:border-white/10 hover:bg-[#111318]/80'}`}
          >
            {p.highlight && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-2 rounded-full bg-[#22A756] text-white text-[10px] font-bold tracking-[0.2em] uppercase shadow-xl z-20">
                Most Featured
              </div>
            )}
            ...
            {p.highlight && (
              <div className="absolute inset-0 bg-gradient-to-b from-[#22A756]/10 to-transparent rounded-[2.5rem] pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            )}
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <ul className="space-y-5 mb-12 flex-grow">
                {p.features.map((f, j) => (
                  <li key={j} className="flex items-start gap-4 group/item">
                    <div className="w-6 h-6 rounded-full bg-[#22A756]/10 flex items-center justify-center shrink-0 mt-0.5 group-hover/item:scale-110 transition-transform">
                      <i className="fa-solid fa-check text-[10px] text-[#22A756]"></i>
                    </div>
              ...
              <a 
                ...
                className={`flex items-center justify-center gap-3 w-full py-5 rounded-[1.25rem] font-bold text-[16px] transition-all tracking-tight shadow-xl
                  ${p.highlight 
                    ? 'bg-[#22A756] text-white hover:bg-[#1e8f49] hover:scale-[1.02]' 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {[
          { icon: 'fa-gift', color: '#22A756', title: 'Free Trial', desc: 'Try Laava free before you subscribe.' },
